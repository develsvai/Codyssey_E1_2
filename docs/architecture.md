...
오류 처리는 "발생한 곳"과 "처리할 책임이 있는 곳"을 나누어 판단한다.

- `core.storage`는 파일 없음, JSON 손상, 스키마 오류처럼 저장소가 직접 판단할 수 있는 문제를 `LoadResult`로 변환한다.
- `core.game`은 퀴즈 추가 후 저장 실패처럼 게임 상태 rollback이 필요한 문제만 처리하고, 저장 실패 자체는 다시 위로 전달한다.
- `cli.app`은 사용자 입력 오류, `KeyboardInterrupt`, `EOFError`처럼 사용자와의 대화 과정에서 처리할 문제를 안내 문구로 바꾼다.
- `main.py`는 CLI 밖으로 빠져나온 예외적인 `KeyboardInterrupt`를 한 번 더 받아 현재 상태 저장을 시도한다.

따라서 모든 예외를 한 곳에 몰아 처리하지 않고, 각 계층이 실제로 복구하거나 안내할 수 있는 범위에서만 처리한다.
...
- CLI 객체 생성
- 프로세스 종료 코드 반환
- 예상하지 못한 `KeyboardInterrupt` 발생 시 현재 상태 저장 후 종료
...
현재 스키마는 미션 요구를 충족하는 최소 형태다. 향후 힌트, 점수 히스토리, 스키마 변경 이력이 필요해지면 `schema_version`, `history`, `hint` 같은 필드를 추가할 수 있다.

## 운영 한계와 확장 여지

현재 구조는 학습용 콘솔 프로그램에 맞춰 단순성을 우선했다.

- 퀴즈 데이터는 실행 시 `list[Quiz]`로 모두 메모리에 올라간다.
- 저장할 때는 `state.json` 전체를 다시 쓴다.
- 퀴즈 목록 검색은 순차 탐색이므로 데이터가 커질수록 검색 비용이 증가한다.

이 방식은 수십 개에서 1000개 정도의 작은 데이터에는 이해하기 쉽고 충분하다. 다만 대량 데이터, 동시 수정, 검색 기능이 중요해지면 SQLite 같은 데이터베이스, 페이지 단위 조회, 검색 인덱스, 저장 파일 백업 전략을 추가하는 것이 적합하다.

백업 전략은 현재 구현하지 않았다. 미션 범위에서는 손상된 파일을 기본 상태로 복구해 프로그램 실행을 유지하는 데 집중했다. 데이터 보존을 더 중요하게 본다면 저장 전 `state.json.bak`을 생성하거나, 정상 저장 성공 후 이전 파일을 별도 보관하는 방식으로 확장할 수 있다.
...
| `KeyboardInterrupt`, `EOFError` 안전 종료 | 충족 | 메뉴, 풀이, 추가 입력 중 발생 시 저장 후 종료하고, `main.py`에서도 최상위 `KeyboardInterrupt`를 한 번 더 처리한다. | 없음. |
...
| 최소 10개 이상 의미 있는 커밋 | 충족 | 현재 `develop` 기준 20개 이상 커밋이 있다. 최신 값은 `git rev-list --count HEAD`로 확인한다. | 없음. |
